...
class AES:
    ...
    def InitialiseMatrix(self, w, h):
        Matrix = [[0 for x in range(w)] for y in range(h)]
        return Matrix

    # Given a string, splits into two bytes and inserts into the matrix
    def InputTextToMatrix(self, inputString):
        w, h = AES.g_Nb, AES.g_Nb
        Matrix = AES.InitialiseMatrix(w, h)
        c = 0
    ...
    def SubBox(self, Matrix):
        for col in range(0, 4):
            for row in range(0, 4):
                tempValue = Matrix[row][col]
                newValue = AES.SubBytesSelection(tempValue)
    ...
    def SubBytesSelection(self, word):
    ...
    def MixColumns(self, Matrix, axMatrix, Nb):
        tempMatrix = AES.InitialiseMatrix(Nb, Nb)
        for col in range(0, 4):
            newValue = 0
            for row in range(0, 4):
    ...
    def SplitKey(self, cipherKey, nk):
        ...
        for i in range(0, len_key, n):
            ...
            AES.g_cipherKeyDic["w{0}".format(index)] = word
            index = index + 1
    ...
    def SplitStringSubBytes(string):
        ...
        for i in range(len(string)):
            if i % 2 == 0:
                ...
                outBytes = AES.SubBytesSelection(inBytes)
    ...
    def KeyExpansion(cK, Nk, Nb, Nr):
        ...
        endRange = Nb * (Nr + 1)
        while i < endRange:
            temp = AES.g_cipherKeyDic["w{0}".format(i - 1)]
            if i % Nk == 0:
                temp = AES.Rotate(temp, 1)
                temp = AES.SplitStringSubBytes(temp)
                rconOut = AES.Rcon(int(i / Nk))
                temp = operator.xor(int(temp, 16), int(rconOut, 16))
                temp = str(hex(temp)[2::])
            elif Nk > 6 and i % Nk == 4:
                temp = AES.SplitStringSubBytes(temp)
            word = int(AES.GetNthValue(i - Nk), 16)
            temp = operator.xor(int(temp, 16), word)
            temp = "{0:#0{1}x}".format(temp, 10)[2::]
            AES.g_cipherKeyDic["w{0}".format(i)] = temp
            i = i + 1
    ...
    def GetNthValue(self, n):
        if n < 0:
            n += len(AES.g_cipherKeyDic)
        for i, value in enumerate(AES.g_cipherKeyDic.values()):
            if i == n:
                return value
    ...
    def CipherKeyWord(n, m):
        temp1 = AES.GetNthValue(n)
        temp2 = AES.GetNthValue(n + 1)
        temp3 = AES.GetNthValue(n + 2)
        temp4 = AES.GetNthValue(n + 3)
        word = temp1 + temp2 + temp3 + temp4
        return word

    # Displays the matrix in order to capture state data
    def DisplayMatrix(self, state):
        newValue = ""
        for row in range(0, 4):
            for col in range(0, 4):
    ...
    def Encrypt(self, cK, Nk, Nb, Nr, inputBytes):
        AES.KeyExpansion(cK, Nk, Nb, Nr)
        state = AES.InputTextToMatrix(inputBytes)
        # WriteTo(" 0 ", " input ", DisplayMatrix(state))
        # WriteTo(" 0 ", " k_sch ", CipherKeyWord(0, Nb - 1))
        state = AES.InputTextToMatrix(AddRoundKey(state, CipherKeyWord(0, Nb - 1)))
        for ROUND in range(1, Nr):
            # WriteTo(ROUND, " start ", DisplayMatrix(state))
            state = AES.SubBox(state)
            # WriteTo(ROUND, " s_box ", DisplayMatrix(state))
            state = AES.ShiftRow(state)
            # WriteTo(ROUND, " s_row ", DisplayMatrix(state))
            state = AES.MixColumns(state, AES.g_axMatrix, Nb)
            # WriteTo(ROUND, " m_col ", DisplayMatrix(state))
            state = AES.InputTextToMatrix(AES.AddRoundKey(state, AES.CipherKeyWord(ROUND * Nb, (ROUND + 1) * (Nb - 1))))
        ...
        state = AES.InputTextToMatrix(AES.AddRoundKey(state, AES.CipherKeyWord(Nr * Nb, (Nr + 1) * (Nb - 1))))
        # WriteTo("14", " k_sch ", CipherKeyWord(Nr * Nb, (Nr + 1) * (Nb - 1)))
        cipherText = AES.DisplayMatrix(state)
